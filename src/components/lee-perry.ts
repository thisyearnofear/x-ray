...
export default class LeePerry {
  ...
  async procesWithCerebras(img: HTMLImageElement): Promise<THREE.Texture | null> {
    try {
      ...
      // Call our secure API route instead of Cerebras directly
      const response = await fetch('/api/cerebras-face', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          imageData,
          prompt: `For face swapping, suggest optimal face region coordinates. Return only JSON: {"x": 0.5, "y": 0.4, "width": 0.7, "height": 0.8} where values are 0-1 representing center-crop percentages for a typical portrait photo.`
        })
      })
      ...
      const { analysis, faceData } = data

      console.log('Cerebras analysis:', analysis)

      if (faceData && faceData.x !== undefined && faceData.y !== undefined) {
