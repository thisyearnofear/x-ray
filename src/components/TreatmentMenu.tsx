...
import { MedicalAction } from '../domains/medical/types';
import { 
  DIAGNOSTIC_TESTS, 
  TREATMENTS, 
  CONSULTATIONS,
...
export const TreatmentMenu: React.FC<TreatmentMenuProps> = ({
  ...
  const [selectedAction, setSelectedAction] = useState<MedicalAction | null>(null);
  ...
  const wasExecuted = (action: MedicalAction) => executedActions.includes(action.id);
    ...
    <div style={styles.overlay} onClick={onClose}>
      <div style={styles.container} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={styles.budgetBanner}>
          ...
          <span style={styles.budgetAmount}>{currentBudget.toFixed(2)} MON</span>
        </div>

        {/* Category Navigation */}
        <div style={styles.categoryNav}>
          {(['all', 'test', 'imaging', 'treatment', 'consultation'] as CategoryType[]).map((category) => (
            <button
              key={category}
              style={{
                ...styles.categoryButton,
                background: selectedCategory === category ? colors.background.primaryGlow : 'transparent',
                borderColor: selectedCategory === category ? colors.primary.base : colors.border.primary,
                color: selectedCategory === category ? colors.primary.base : colors.neutral.light
              }}
        ...
        <div style={styles.content}>
          <div style={styles.actionGrid}>
            {filteredActions.map((action) => {
              const affordable = isAffordable(action);
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    borderColor: executed ? colors.primary.base : colors.border.primary
                  }}
                  ...
                  onMouseLeave={(e) => {
                    ...
                    e.currentTarget.style.borderColor = executed ? colors.primary.base : colors.border.primary;
                ...
                >
                  ...
                  <div style={styles.actionContent}>
                    <div style={styles.actionHeader}>
                      ...
                      <span style={{
                        ...styles.actionCost,
                        color: affordable ? colors.primary.base : colors.error.base
                      }}>
                        {action.cost.toFixed(2)} MON
                      </span>
                    </div>
            ...
            >
              ...
              <div style={{ ...styles.content, padding: spacing.xl }}>
                <div style={styles.detailSection}>
                  ...
                  <div style={{...styles.detailText, fontSize: typography.fontSize['2xl'], color: colors.primary.base, fontFamily: typography.fontFamily.monospace}}>
                    {selectedAction.cost.toFixed(2)} MON
                  </div>
                </div>

                <div style={styles.detailSection}>
                  <div style={styles.detailLabel}>Expected Outcome</div>
                  <div style={styles.detailText}>{selectedAction.expectedOutcome}</div>
                </div>
                ...
                <div style={styles.detailSection}>
                  ...
                  <ul style={styles.riskList}>
                    {selectedAction.risks.map((risk, i) => (
                      <li key={i} style={styles.riskItem}>⚠️ {risk}</li>
                    ))}
                  </ul>
                </div>

                {selectedAction.contraindications && selectedAction.contraindications.length > 0 && (
                  <div style={styles.detailSection}>
                    <div style={styles.detailLabel}>Contraindications</div>
                    <ul style={styles.riskList}>
                      {selectedAction.contraindications.map((contra, i) => (
                        <li key={i} style={styles.riskItem}>🚫 {contra}</li>
                      ))}
                    </ul>
                  </div>
                )}
                ...
                <button
                  style={{
                    ...
                    opacity: isAffordable(selectedAction) ? 1 : 0.5,
                    cursor: isAffordable(selectedAction) ? 'pointer' : 'not-allowed'
                  }}
                  disabled={!isAffordable(selectedAction)}
                  onClick={() => {
                    if (isAffordable(selectedAction)) {
                ...
                >
                  {isAffordable(selectedAction) ? 'Execute Action' : 'Insufficient Funds'}
                </button>
              </div>
            </div>
          </div>
